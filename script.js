...
function card(product, fisher = false) {
  ...
  const closed = left <= 0 || status === '입항 완료';
  ...
  return `<article class="product" style="border-color:${closed ? '#f0bdb1' : '#bde8d0'};background:${closed ? '#fff7f5' : '#f4fcf7'}"><div class="product-head">${product.photo ? `<img class="product-image" src="${product.photo}" alt="${escapeHtml(product.name)} 사진">` : '<span class="fish-icon">🐟</span>'}<div><h4>${escapeHtml(product.name)}</h4><p class="boat">${escapeHtml(product.boat)} · 입항시간 ${dateText(product.arrival)}</p><p class="boat">판매자 ${sellerText(product)}</p></div><span class="status" style="background:${closed ? '#fff0ed' : '#e5f7ee'};color:${closed ? '#d85c38' : '#16805b'}">${closed ? '예약 마감' : '예약 가능'}</span></div><div class="meta"><span>가격 <strong>${priceText(product)}</strong></span><span>남은 수량 <strong>${quantityText(left, product)}</strong></span></div><p class="pickup">📍 픽업: ${escapeHtml(product.pickup || '픽업 장소 미정')}</p>${product.phone ? `<p class="pickup">📞 문의: <a href="tel:${escapeHtml(product.phone)}">${escapeHtml(product.phone)}</a></p>` : ''}${product.message ? `<p class="seller-message">💬 ${escapeHtml(product.message)}</p>` : ''}<div class="progress"><i style="width:${percent}%"></i></div><div class="product-footer"><span>예약 ${quantityText(product.reserved, product)} / 전체 ${quantityText(product.quantity, product)}</span>${fisher ? fisherControls : reserve}</div></article>`;
}
...
function renderCitizen() {
  const search = ($('productSearch')?.value || '').trim().toLowerCase(), list = products().filter(p => !search || `${p.name} ${p.boat} ${p.type || ''}`.toLowerCase().includes(search)).sort((a, b) => { const aClosed = remaining(a) <= 0 || a.status === '입항 완료'; const bClosed = remaining(b) <= 0 || b.status === '입항 완료'; return Number(aClosed) - Number(bClosed); });
...
async function addProduct(event) { event.preventDefault(); const form = event.target, file = $('productPhoto').files[0]; if (file && file.size > 500 * 1024) return toast('사진은 500KB 이하만 등록할 수 있습니다.'); const photo = file ? await fileToDataUrl(file) : null, all = marketState.products || [], editing = form.dataset.editing;
  const data = { boat:$('boatName').value.trim(), name:$('productName').value.trim(), type:$('productType').value, price:Number($('productPrice').value), quantity:Number($('productQuantity').value), unit:$('quantityUnit').value, arrival:$('arrivalTime').value, pickup:$('pickupLocation').value.trim(), phone:$('sellerPhone').value.trim(), message:$('sellerMessage').value.trim() };
