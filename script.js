...
function card(product, fisher = false) {
  ...
  const reserve = left > 0 && status !== '입항 완료' ? `<div class="reserve-row"><input id="qty-${product.id}" type="number" min="1" max="${left}" value="1" aria-label="예약 수량"><button class="primary small" onclick="reserveProduct('${product.id}')">예약하기</button></div>` : '<b>예약 마감</b>';
  return `<article class="product"><div class="product-head">${product.photo ? `<img class="product-image" src="${product.photo}" alt="${escapeHtml(product.name)} 사진">` : '<span class="fish-icon">🐟</span>'}<div><h4>${escapeHtml(product.name)}</h4><p class="boat">${escapeHtml(product.boat)} · ${dateText(product.arrival)}</p></div><span class="status ${status === '입항 완료' ? 'arrived':''}">${status}</span></div><div class="meta"><span>가격 <strong>${won(product.price)}</strong></span><span>남은 수량 <strong>${left}개</strong></span></div><p class="pickup">📍 픽업: ${escapeHtml(product.pickup || '픽업 장소 미정')}</p><div class="progress"><i style="width:${percent}%"></i></div><div class="product-footer"><span>예약 ${product.reserved || 0}개 / 전체 ${product.quantity}개</span>${fisher ? fisherControls : reserve}</div></article>`;
}
function renderFisher() {
  ...
  $('fisherSummary').innerHTML = `<div class="summary"><span>예약 수량</span><strong>${reserved}개</strong></div><div class="summary"><span>남은 재고</span><strong>${stock}개</strong></div><div class="summary"><span>예상 매출</span><strong>${won(sales)}</strong></div>`;
...
function reservationRow(reservation, past) { const arrival = arrivalOf(reservation), cancellable = arrival && Date.now() < new Date(arrival).getTime() - 3600000; return `<div class="reservation ${past ? 'past-reservation':''}"><div><strong>${escapeHtml(reservation.name)} <small>${escapeHtml(reservation.boat)} · ${reservation.quantity}개 · ${arrival ? dateText(arrival) : ''}</small></strong></div><div class="reservation-actions"><b>${won(reservation.total)}</b>${past ? '' : `<button class="cancel" ${cancellable ? '' : 'disabled'} onclick="cancelReservation('${reservation.id}')">${cancellable ? '예약 취소' : '취소 마감'}</button>`}</div></div>`; }
...
async function addProduct(event) { event.preventDefault(); const form = event.target, file = $('productPhoto').files[0]; if (file && file.size > 500 * 1024) return toast('사진은 500KB 이하만 등록할 수 있습니다.'); const photo = file ? await fileToDataUrl(file) : null, all = marketState.products || [], editing = form.dataset.editing;
  const data = { boat:$('boatName').value.trim(), name:$('productName').value.trim(), type:$('productType').value, price:Number($('productPrice').value), quantity:Number($('productQuantity').value), arrival:$('arrivalTime').value, pickup:$('pickupLocation').value.trim() };
...
function reserveProduct(id) { const product=(marketState.products || []).find(p=>p.id===id), qty=Number($(`qty-${id}`).value); if (!product || qty < 1 || qty > remaining(product)) return toast('남은 수량 안에서 선택하세요.'); product.reserved=Number(product.reserved || 0)+qty; marketState.reservations=[{id:crypto.randomUUID(),productId:id,user:currentName(),userId:currentId(),name:product.name,boat:product.boat,quantity:qty,total:qty*product.price,arrival:product.arrival},...reservations()]; save(); renderAll(); toast('예약되었습니다.'); }
...
Object.assign(window,{setAuthMode,togglePassword,submitAuth,startAs,goHome,logout,previewPhoto,toggleProductForm,addProduct,editProduct,updateStatus,reserveProduct,cancelReservation,renderCitizen});
document.addEventListener('DOMContentLoaded',()=>{ const date=new Date(Date.now()+3*3600000); if ($('arrivalTime')) $('arrivalTime').value=date.toISOString().slice(0,16); onAuthStateChanged(auth,user=>{ signedInUser=user; renderAll(); }); getDoc(marketRef).then(snapshot=>{ if (!snapshot.exists()) return setDoc(marketRef,marketState); }).catch(()=>toast('Firebase 연결을 확인하세요.')); onSnapshot(marketRef,snapshot=>{ if(snapshot.exists()){ marketState=snapshot.data(); renderAll(); } }); });
